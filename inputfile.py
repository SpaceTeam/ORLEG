# -*- coding: utf-8 -*-
"""
Created on Fri Aug 31 21:44:08 2018
...
sw={'H2O':2.3579*10**-1,'CO2':5.5019*10**-2,'CO':1.7712*10**-1,'Oxygen':0,'Nitrogen':4.3*10**-1,'Hydrogen':1.0656*10**-1,'Methane':0,'NH3':0}
...
#Calculation of fuel tank mass per length
...
#Inputconverter (mole to mass)

def conversion(werte):
    count=0
    for key in werte:
        count +=sw[key]
    if count <1+0.008 and count > 1-0.008:
        None
    else:
...
if datainput=='F':
    Tch,sw,Pch=cd.ceaextractor(ceainput)
elif datainput=='M':
    None
else:
    print('invalid datainputswitch')   
a=conversion(sw)
mtl=mpl(dt)
