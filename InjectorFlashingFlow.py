...
class InjectorFlashingFlow(object):
    def __init__(
        ...
        self.v_gs = 1 / ps(
            "D", "Q", 1, "T", self.liquid_temperature, self.fluid
        )  # m3/kg specific volume of gas at standard pressure
        self.deltah_vs = ps("H", "Q", 1, "T", self.liquid_temperature, self.fluid) - ps(
            "H", "Q", 0, "T", self.liquid_temperature, self.fluid
        )  # J/kg Verdampfungsenthalpie am Eintritt
    ...
    def updateTankPressure(self, tank_pressure):
        ...
        v_g0 = 1 / ps(
            "D", "Q", 1, "P", tank_pressure, self.fluid
        )  # m3/kg specific volume of gas at tank pressure
        self.v_l0 = 1 / ps(
            "D", "P", tank_pressure, "T", self.liquid_temperature, self.fluid
        )  # m3/kg specific volume of liquid at tank pressure

        self.a = (
            (7.5 / ((self.orifice_length / self.orifice_diameter) + 7.5))
            * (self.eta_s ** (-0.6))
        ) + ((3 / 5) * self.gamma)
        ...
        self.omega_frozen = 0
        self.omega_flash = (
            self.cp_ls * self.liquid_temperature * tank_pressure * self.eta_s / self.v_0
        ) * (((self.v_gs - self.v_l0) ** 2) / (self.deltah_vs**2))
    ...
    def iteration(self, eta_crit):
        xdot_eqth = self.xdot_0 + (
            self.omega_flash
            * (self.v_0 / (self.v_gs - self.v_l0))
            * math.log(self.eta_s / eta_crit)
        )
        N = xdot_eqth**self.a
        ...
        Gamma2 = (
            (
                ...
                * (
                    ...
                    - 1
                    - (
                        (self.xdot_0 / self.omega_flash)
                        * ((self.v_gs - self.v_l0) / self.v_0)
                        * (1 + self.omega)
                    )
                )
            )
            + ((self.a / 2) * (self.omega - self.omega_frozen) * (1 + self.omega))
            - (self.omega**2)
            + 1
        )
        Gamma3 = (
            (self.omega * ((1 / self.eta_s) - 1))
            - 0.5
            + (
                (
                    (
                    ...
                    - ((self.a * (self.omega - self.omega_frozen)) / Omega)
                )
                * ((Omega / 2) - (self.omega**2))
            )
        )
        return (
            self.eta_s
            * (
                ...
                + (
                    (1 / (2 * Gamma1))
                    * (Gamma2 - math.sqrt((Gamma2**2) - (4 * Gamma1 * Gamma3)))
                )
            )
        ) - eta_crit  # subtract eta_crit to make function return zero when correct eta_crit found
    ...
    def getMassFlow(self, tank_pressure):
        ...
        C = math.sqrt(
            (1 - self.eta_s)
            + (self.omega * self.eta_s * math.log(self.eta_s / self.eta_crit))
            - ((self.omega - 1) * (self.eta_s - self.eta_crit))
        ) / ((self.omega * ((self.eta_s / self.eta_crit) - 1)) + 1)
        ...
        return (
            ...
            * C
            * math.sqrt(2 * tank_pressure / self.v_l0)
        )
...
if __name__ == "__main__":
    ...
    # calculate mass flow
    injector = InjectorFlashingFlow(
        fluid="N2O",
        vapor_pressure=30e5,
        orifice_length=11e-3,
        orifice_diameter=1.4e-3,
        orifice_count=4,
        discharge_coefficient=0.8,
    )
    ...
    print(
        "mass flow with",
        injector.orifice_count,
        "orifices of",
        injector.orifice_diameter * 1e3,
        "mm diameter at",
        injector.tank_pressure * 1e-5,
        "bar tank pressure:",
        round(massFlow, 3),
        "kg/s",
    )
    ...
    referenceInjector = InjectorFlashingFlow(
        fluid="N2O",
        vapor_pressure=40e5,
        orifice_length=18.4e-3,
        orifice_diameter=1.5e-3,
        orifice_count=1,
        discharge_coefficient=0.8,
    )
    ...
    ref_tank_pressures = [
        referenceInjector.vapor_pressure + (pressure_psi / 14.5038 * 1e5)
        for pressure_psi in ref_supercharge_pressures_psi
    ]

    ref_massflows_calculated = []

    for ref_tank_pressure in ref_tank_pressures:
        ref_massflows_calculated.append(
            referenceInjector.getMassFlow(ref_tank_pressure)
        )

    plt.plot(
        ref_tank_pressures, ref_massflows_calculated, ref_tank_pressures, ref_massflows
    )
